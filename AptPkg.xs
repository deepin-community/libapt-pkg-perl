...
#define pkgCache_DescIterator_p		parented<pkgCache::DescIterator>
...
#define pkgCache_DescFileIterator_p	parented<pkgCache::DescFileIterator>
...
#define CONSTANT(_sub_, _enum_) \
    do { \
	CV *cv = newXS(_sub_, XS_AptPkg__constant, file); \
  ...
  PPCODE:
	...
	for (I32 i = 0; i < len; i++)
	{
	    ...
	    if ((e = parse_avref(aTHX_ av_fetch(av, i, 0), "czs|s",
				 ...
				 &a[j].ConfName, &type)))
		warn("AptPkg::_parse_cmdline: invalid array %ld (%s)", i, e);
	    else
...
bool
pkgCacheFile::MultiArchCache()
  CODE:
    pkgCache *cache = *THIS;
    RETVAL = cache->MultiArchCache();

  OUTPUT:
    RETVAL

char *
pkgCacheFile::NativeArch()
  CODE:
    pkgCache *cache = *THIS;
    RETVAL = (char *) cache->NativeArch();
...
string
pkgCache_PkgIterator_p::FullName(Pretty = false)
    bool Pretty
  CODE:
    RETVAL = THIS->obj->FullName(Pretty);

  OUTPUT:
    RETVAL

char *
pkgCache_PkgIterator_p::Arch()
  CODE:
...
pkgCache_PkgIterator_p::Section()
  CODE:
    RETVAL = (char *) (THIS->obj->VersionList()
	? THIS->obj->VersionList().Section()
	: 0);
...
pkgCache_PkgIterator_p::CurrentState()
  ...
  CODE:
    ...
    case pkgCache::State::TriggersAwaited:  rv = "TriggersAwaited";	break;
    case pkgCache::State::TriggersPending:  rv = "TriggersPending";	break;
    default:				    XSRETURN_UNDEF;
    }
...
SV *
pkgCache_VerIterator_p::MultiArch()
  PREINIT:
    char const *rv;

  CODE:
    switch ((*THIS->obj)->MultiArch)
    {
    case pkgCache::Version::No:		    rv = "No";			break;
    case pkgCache::Version::All:	    rv = "All";			break;
    case pkgCache::Version::Foreign:	    rv = "Foreign";		break;
    case pkgCache::Version::Same:	    rv = "Same";		break;
    case pkgCache::Version::Allowed:	    rv = "Allowed";		break;
    case pkgCache::Version::AllForeign:	    rv = "AllForeign";		break;
    case pkgCache::Version::AllAllowed:	    rv = "AllAllowed";		break;
    default:				    XSRETURN_UNDEF;
    }

    RETVAL = newSViv((*THIS->obj)->MultiArch);
    sv_setpv(RETVAL, rv);
    SvIOK_on(RETVAL);
...
SV *
pkgCache_VerIterator_p::DescriptionList()
  PPCODE:
    for (pkgCache::DescIterator i = THIS->obj->DescriptionList(); !i.end(); ++i)
    {
	pkgCache_DescIterator_p *d = new pkgCache_DescIterator_p(THIS_sv, i);
	SV *dep = sv_newmortal();
	sv_setref_pv(dep, "AptPkg::Cache::_description", (void *) d);
	XPUSHs(dep);
    }

pkgCache_DescIterator_p *
pkgCache_VerIterator_p::TranslatedDescription()
  CODE:
    pkgCache::DescIterator i = THIS->obj->TranslatedDescription();
    RETVAL = new pkgCache_DescIterator_p(THIS_sv, i);
...
MODULE = AptPkg  PACKAGE = AptPkg::Cache::_description

void
pkgCache_DescIterator_p::DESTROY()

char *
pkgCache_DescIterator_p::LanguageCode()
  CODE:
    RETVAL = (char *) THIS->obj->LanguageCode();

  OUTPUT:
    RETVAL

char *
pkgCache_DescIterator_p::md5()
  CODE:
    RETVAL = (char *) THIS->obj->md5();

  OUTPUT:
    RETVAL

pkgCache_DescFileIterator_p *
pkgCache_DescIterator_p::FileList()
  CODE:
    pkgCache::DescFileIterator i = THIS->obj->FileList();
    RETVAL = new pkgCache_DescFileIterator_p(THIS_sv, i);
...
MODULE = AptPkg  PACKAGE = AptPkg::Cache::_desc_file

void
pkgCache_DescFileIterator_p::DESTROY()

pkgCache_PkgFileIterator_p *
pkgCache_DescFileIterator_p::File()
  CODE:
    pkgCache::PkgFileIterator i = THIS->obj->File();
    RETVAL = new pkgCache_PkgFileIterator_p(THIS_sv, i);
...
#define PUSH_PAIR(_pack_, _name_) \
    do { \
	string v; \
	if ((v = _pack_._name_()).size()) \
	{ \
	    EXTEND(SP, 2); \
...
SV *
pkgRecords_p::Lookup(arg)
    SV *arg
  PPCODE:
    pkgCache_VerFileIterator_p *pack = 0;
    pkgCache_DescFileIterator_p *desc = 0;

    if (SvROK(arg))
    {
	if (sv_derived_from(arg, "AptPkg::Cache::_ver_file"))
	    pack = (pkgCache_VerFileIterator_p *) SvIV((SV *) SvRV(arg));
	else if (sv_derived_from(arg, "AptPkg::Cache::_desc_file"))
	    desc = (pkgCache_DescFileIterator_p *) SvIV((SV *) SvRV(arg));
    }

    if (pack)
    {
	pkgRecords::Parser &p = THIS->obj->Lookup(*pack->obj);
	PUSH_PAIR(p, FileName);
	PUSH_PAIR(p, SourcePkg);
	PUSH_PAIR(p, Maintainer);
	PUSH_PAIR(p, ShortDesc);
	PUSH_PAIR(p, LongDesc);
	PUSH_PAIR(p, Name);

	for (const auto &hash : p.Hashes())
	{
	    string type = hash.HashType();
	    if (type == "MD5Sum")
		type = "MD5Hash";  /* keep old key for MD5Sum */

	    EXTEND(SP, 2);
	    PUSHs(sv_2mortal(newSVpvn(type.c_str(), type.size())));
	    PUSHs(sv_2mortal(newSVpvn(hash.HashValue().c_str(),
		hash.HashValue().size())));
	}
    }
    else if (desc)
    {
	pkgRecords::Parser &p = THIS->obj->Lookup(*desc->obj);
	PUSH_PAIR(p, ShortDesc);
	PUSH_PAIR(p, LongDesc);
	PUSH_PAIR(p, Name);
    }
    else
	croak("arg is not of type AptPkg::Cache::_ver_file"
	    " or AptPkg::Cache::_desc_file");
...
pkgSrcRecords::Find(src, src_only = false)
  ...
  PPCODE:
    ...
    vector<pkgSrcRecords::Parser::BuildDepRec> build_depends;
    if (parser->BuildDepends(build_depends, false))
    {
	HV *hv = newHV();
	for (const auto &b : build_depends)
	{
	...
	for (const auto &f : files)
	{
	    ...
	    for (const auto &hash : f.Hashes)
	    {
		string type = hash.HashType();
		if (type == "MD5Sum")
		    type = "MD5Hash";  /* keep old key for MD5Sum */

		hv_store(hv, type.c_str(), type.size(),
		    newSVpvn(hash.HashValue().c_str(), hash.HashValue().size()),
		    0);
	    }

	    /* note that the key is "Size" rather than "FileSize" to
	       preserve existing behaviour */
	    hv_store(hv, "Size", 4, newSVuv(f.FileSize), 0);
	    hv_store(hv, "ArchiveURI", 10,
		newSVpv(parser->Index().ArchiveURI(f.Path).c_str(), 0), 0);

	    hv_store(hv, "Type", 4, newSVpvn(f.Type.c_str(), f.Type.size()), 0);
...
BOOT:
	...
	CONSTANT("AptPkg::Version::No",		pkgCache::Version::No);
	CONSTANT("AptPkg::Version::All",	pkgCache::Version::All);
	CONSTANT("AptPkg::Version::Foreign",	pkgCache::Version::Foreign);
	CONSTANT("AptPkg::Version::Same",	pkgCache::Version::Same);
	CONSTANT("AptPkg::Version::Allowed",	pkgCache::Version::Allowed);
	CONSTANT("AptPkg::Version::AllForeign", pkgCache::Version::AllForeign);
	CONSTANT("AptPkg::Version::AllAllowed", pkgCache::Version::AllAllowed);
	/* deprecated variant of No */
	CONSTANT("AptPkg::Version::None",	pkgCache::Version::No);
	...
	CONSTANT("AptPkg::Dep::DpkgBreaks",	pkgCache::Dep::DpkgBreaks);
	CONSTANT("AptPkg::Dep::Enhances",	pkgCache::Dep::Enhances);
	/* variant of DpkgBreaks to match the keyword */
	CONSTANT("AptPkg::Dep::Breaks",		pkgCache::Dep::DpkgBreaks);
	...
	CONSTANT("AptPkg::State::HalfConfigured",
	...
	CONSTANT("AptPkg::State::TriggersAwaited",
	    pkgCache::State::TriggersAwaited);

	CONSTANT("AptPkg::State::TriggersPending",
